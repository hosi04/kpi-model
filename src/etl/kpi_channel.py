    ...
    def calculate_kpi_day_channel(
        ...
        for row in kpi_day_channel_data:
            ...
            if calendar_date <= today:
                ...
                gap = actual - float(kpi_channel_initial)
            else:
                gap = 0
                kpi_day_adjustment = kpi_day_adjustment_by_date.get(calendar_date)
                if kpi_day_adjustment is not None:
                    kpi_channel_adjustment = float(kpi_day_adjustment) * float(rev_pct_adjustment)
                else:
            ...
            results.append({
                ...
                'kpi_channel_initial': float(kpi_channel_initial),
                'actual': actual,
                'gap': gap,
